...
Dabei sind alle vierzig \textit{nice}-Werte mit entsprechenden Gewichten der Reihe nach absteigend dargestellt.
...
Die Abstände zwischen den Zahlen sind so gewählt, dass bei einem Auf- bzw. Absteigen eines \textit{nice}-Wertes die CPU-Zuteilungszeit um 10\% erhöht bzw. um 10\% erniedrigt wird. Damit ergibt sich ein annähernder Faktor zwischen den Gewichten von ca. 1,25. Dies kann im Kernel unter \texttt{sched.c} an entsprechender Stelle eingesehen werden. 
...
\textit{period} ist ein Abschnitt, für welchen die Prozesse ihren entsprechenden proportionalen Zeitanteil erhalten. Dieser Wert ist einstellbar.

Die maximal gültige Anzahl von Prozessen für diesen Zeitabschnitt kann z.B. mit der Division der aktuellen \textit{period} durch den Wert der kleinst möglichen Zeitscheibe, enthalten im Wert von  \texttt{sysctl\_sched\_min\_granulari\-ty}, ermittelt werden.  
%Errechnet man z.B. die maximal gültige Anzahl von Prozessen für diesen Zeitabschnitt mit $period %/   und 
Übersteigt die Anzahl der aktuellen Prozesse diesen Wert, so muss \textit{period} entsprechend angepasst werden.
...
Im anschließenden Schritt soll eine faire Auswahl auf den nächsten ausführbaren Prozess getroffen werden. Dies geschieht mit einer weiteren Bewertung der Prozesse im Bezug zu ihrer bereits ausgeführten Zeit. 
...
Stehen zum Beispiel drei lauffähige Prozesse  mit \textit{nice}-Werten von  $0$, $1$ und $2$ und entsprechenden Gewichten von $1024$, $820$ und $655$ in einer Warteschlange für einen Prozessor, dann ergeben sich die idealen Zeitscheiben bei einer Periode von 20ms mit Formel (\ref{eq:slice}) zu $8.2ms$, $6.6ms$ und $5.2ms$.
...
Bei \textit{nice}-Wert 0 ergäbe es nach Einsetzten des oben berechneten \textit{slice}-Wertes von $8.2ms$ für \textit{delta\_exec} in Formel (\ref{eq:vruntime}), dem ermittelten Gewicht von $1024$ für \textit{se$\rightarrow$load.weight} und $NICE\_\\0\_LOAD$ von 1024 einen Wert von $8,2ms$.
Für \textit{nice}-Wert 1 wird eine  \textit{virtual runtime} von ebenfalls $8,2ms$ erreicht und für \textit{nice} 2 ein Wert von $7,9ms$. 

Damit wird gezeigt, dass der \textit{virtual runtime}-Wert dazu verwendet werden kann, um eine gerechte Aufteilung entsprechend der \textit{nice}-Werte zu ermöglichen. 
Sind alle \textit{virtual runtime}-Werte nach Abschluss der Periode gleich, so wird das \textit{completely-fair} Prinzip erreicht.
