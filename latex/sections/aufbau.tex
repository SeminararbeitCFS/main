...
Unter Linux und unixartigen Systemen werden daher den beiden Prozessen Zeitscheiben zugeordnet und es findet eine sequentielle Abarbeitung statt. Anschließend werden die Prozesse abwechselnd mit jeweils der maximal zur Ver\-fügung stehenden Prozessorressource entsprechend ihrer Zeitscheibe verarbeitet.
...
In dieser Tabelle ist die entsprechende Zuweisung vom \textit{nice}-Wert zum Gewicht dargestellt. 

Dabei sind pro Zeile immer fünf Zuweisungen von \textit{nice}-Werten zu Gewichten zu sehen, welche bei dem \textit{nice}-Wert der ersten Spalte starten und nach rechts immer um eins inkrementiert wird. 
